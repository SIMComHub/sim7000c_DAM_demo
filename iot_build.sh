...
TS_IOTKIT_SRC_PATH="./src/app/src/iotkit"
TS_IOTKIT_INC_PATH="./src/app/inc/iotkit"
...
export DAM_INCPATHS="-I $DAM_INC_BASE -I $DAM_INC_BASE/threadx_api -I $DAM_INC_BASE/qapi -I $TS_APP_INC_PATH -I $TS_IOTKIT_INC_PATH"
...
$TOOLCHAIN_PATH/armcc -g -c $DAM_CPPFLAGS $DAM_CFLAGS $APP_CFLAGS $DAM_INCPATHS $TS_APP_SRC_PATH/*.c $TS_IOTKIT_SRC_PATH/*.c


mv *.o $TS_APP_OUTPUT_PATH

echo "Linking CustApp application"
$TOOLCHAIN_PATH/armlink -d -o $DAM_OUTPUT_PATH/$DAM_ELF_NAME --elf --ro $DAM_RO_BASE --first txm_module_preamble.o --entry=_txm_module_thread_shell_entry --map --remove --symbols --list $DAM_OUTPUT_PATH/dam_app.map $TS_APP_OUTPUT_PATH/*.o  $DAM_LIB_PATH/$DAM_LIBNAME $DAM_LIB_PATH/$TIMER_LIBNAME $DAM_LIB_PATH/libiot_sdk.a
